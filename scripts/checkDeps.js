          ...
          if (!importPath.startsWith('.') && path.node.importKind !== 'type') {
            ...
            try {
              ...
              if (peerDependencies.every((item) => !importPath.startsWith(item))) {
                ...
                const packageJson = require(slash(join(__dirname, '..', packagePath, 'package.json')))
                if (!JSON.stringify(packageJson.dependencies).includes(packageName)) {
                  resolve({
                    success: false,
                    message: `${packagePath} 的 ${packageName} 依赖没有在 ${slash(join(__dirname, '..', packagePath, 'package.json'))} 中申明`
                  })
                  return
                }
              }
...
const checkDeps = ({ cwd }) => {
  ...
  const tsFiles = globList(['packages/**/src/**/*.tsx', 'packages/**/src/**/*.tsx'], {
    cwd,
    ignore: ['**/*.d.ts', '**/demos/**', '**/dist/**', '**/public/**', '**/locales/**', '**/node_modules/**']
  })
